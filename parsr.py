...
def sentiment_labeling(df):
        '''
        Функция обогащает передаваемый датасет с текстовыми сообщениями разметкой тональности каждого из них
        (позитивный/негативный/нейтральный). Возвращает обогащенный датасет.
        '''
        sentiment_labels = []
...
def save_results(df, link):
        '''
        Записываем в csv файл результаты. Если чаты уже парсились ранее и есть какие-то
        сообщения в соответствующих файлах, происходит "склейка" новой порции и старой из файла.
        А потом уже весь собранный датафрейм записывается в тот же старый файл.
        '''
...
def json_out(link, msg_limit=10):
        '''
        Для вывода через API в виде json
        '''
