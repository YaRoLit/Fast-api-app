...
alt.themes.enable('streamlit')      # Этот параметр сильно меняет цветовую раскладку, но почему-то без неё не запускается интерактив
...
if uploaded_file is not None:
    ...
    dn = dn.to_frame().reset_index()    # Альтаировская (да и другая) отрисовка не корректно работает с Series, переводим в Frame
    ...
    chart = (
        ...
        .interactive()              # Этот параметр лобавляет отображение даты при наведении на пик на графике
    )
